@ECHO OFF
set "prj=tst29"
:: set prj MUST in LINE 2 :: ws edit src prj-name-dir ::
REM *** IMPORTANT: The first 3 lines (including "set prj=...") used by  external software it interacts with.
rem !!!DO NOT CHANGE!!! ***
rem 1 barePUSH
SETLOCAL
REM *** only push . must exist: REPO, BARE-REPO und REMOTE  ***
rem --- Globale Konfiguration (ReadOnly) ---
set "REPO_DIR=D:\ws\gitGit\source\workspaces\%prj%"
set "BARE_DIR=D:\ws\gitGit\source\local-bare-repos\%prj%.git"
set "BRANCH=main"
set "REMOTE=origin"
set "FOLDERS=EDsrc"
set "KEY_PATH=%USERPROFILE%\.ssh\id_ed25519"

REM Die vollstaendige URL wird nur zur Anzeige benoetigt, falls die Remote-Logik wieder aktiv wird.
set "GITHUB_USER_URL=https://github.com/e6000000"
set "FULL_REMOTE_URL=%GITHUB_USER_URL%/%prj%"


rem *** ENTFERNT: mkdir %REPO_DIR% und %BARE_DIR% ***


REM Check if a custom commit message was passed
if "%COMMIT_MSG%"=="" (
    set "COMMIT_MSG=Auto-Commit %DATE% %TIME% see comments in file OLDCODE: NEWCODE: "
)

rem -------------------------------------------------------
rem 1) TAG GENERATION WITH PSEUDO-VERSIONING (Code Block)
rem -------------------------------------------------------
set "YY=%DATE:~2,2%"
set "MON=%DATE:~5,2%"
set "DAY=%DATE:~8,2%"
set "H=%TIME:~0,2%"
if "%H:~0,1%"==" " set "H=0%H:~1,1%"
set "MIN=%TIME:~3,2%"
set "SEC=%TIME:~6,2%"
set /a BASEYY=24
set /a yearssince = YY - BASEYY
if %yearssince% Lss 0 set /a yearssince=0
set /a SEC_DAY=24*3600
set /a SEC_MONTH=30*SEC_DAY
set /a SEC_YEAR=360*SEC_DAY
set /a mon = 1%MON% - 1
set /a day = 1%DAY% - 1
set /a hour = 1%H% - 1
set /a minute = 1%MIN% - 1
set /a second = 1%SEC% - 1
set /a total = yearssince*SEC_YEAR + mon*SEC_MONTH + day*SEC_DAY + hour*3600 + minute*60 + second
set /a step = (total / 50) %% 1000000000
set /a step = step   -6521726
rem add /50 because it is ok no need tooo big numbers qq66yy
set /a step = step + 1000000
echo ver NR.    %step%
set "num=000000000%step%"
set "num=%num:~-9%"
rem "version=datim_ 890
set "version=barepustim_%num:~0,3%.%num:~3,3%.%num:~6,3%"
echo version: %version%
set "TAG_NAME=%version%"
echo Generated Tag Name: %TAG_NAME%
set "TAG_MSG_USER=[2033]"
echo.
set /p TAG_MSG_USER="Enter a Tag Message (Default: Release %TAG_NAME%): "
if "%TAG_MSG_USER%"=="" (
    set "TAG_MSG_USER=Release %TAG_NAME%"
)
echo _ _
echo  _ prj: %prj%    _ branch: %branch%
echo  _ TAG_MSG_USER: %TAG_MSG_USER%   _ TAG_NAME: %TAG_NAME%
echo _ _
rem -------------------------------------------------------

rem Change to Repo directory (Muss existieren!)
cd /d "%REPO_DIR%"

rem (Optional) Set Git ssH-Command
set "GIT_ssH_COMMAND=ssh -i "%KEY_PATH%" -o IdentitiesOnly=yes"

rem *** ENTFERNT: BARE REPO INITIALISIERUNG ***
rem *** ENTFERNT: REPO INITIALISIERUNG (git init) ***
rem *** ENTFERNT: BRANCH UMBENENNUNGS-LOGIK (da 'main' nun Standard ist) ***
rem *** ENTFERNT: REMOTE ADD/SET URL LOGIK (da 'origin' nun existiert) ***


rem Add specified folders
for %%F in (%FOLDERS%) do (
    echo git add "%%F"
    git add "%%F"
)

rem *** ENTFERNT: AUFRUF DER AutoIt EXE (nur fuer Erst-Erstellung) ***


rem Check if anything is staged, and CREATE COMMIT / TAG (Part 1)
git diff --cached --quiet
if errorlevel 1 (
    echo Changes are staged, creating Commit...
    git commit -m "%COMMIT_MSG%"
    if errorlevel 1 (
        echo Commit failed.
        timeout /t 222
        exit /b 1
    )
    echo Commit successful.
    echo.

    rem Create tag with custom message
    echo  * creating Tag %TAG_NAME% with message "%TAG_MSG_USER%"...
    git tag -a "%TAG_NAME%" -m "%TAG_MSG_USER%"
    if errorlevel 1 (
        echo Tag creation failed.
        timeout /t 222
        exit /b 1
    )
    echo Tag %TAG_NAME% successfully created.
    echo.

    rem #######################################################
    rem START: BARE-REPO PUSH (Part 1: Push to local Bare Repo)
    rem #######################################################
    echo WARNING: Now pushing to local Bare Repo %BARE_DIR%.
    if /I "y"=="y" (
        echo Pushing Branch %BRANCH% to %BARE_DIR% ...
        git push "%BARE_DIR%" %BRANCH%
        if errorlevel 1 (
            echo Push of Branch to BARE_DIR failed.
            timeout /t 222
            exit /b 1
        )

        rem Push Tags
        echo Pushing Tag %TAG_NAME% to %BARE_DIR%...
        git push "%BARE_DIR%" "%TAG_NAME%"
        if errorlevel 1 (
            echo Push of Tag to BARE_DIR failed.
            timeout /t 222
            exit /b 1
        )

        echo  ..
        echo ########################################
        echo Push and Tag Push to BARE_DIR %prj% successful.
        echo  ..

    ) else (
        echo BARE Push aborted.
    )

    rem #######################################################
    rem START: REMOTE PUSH (Part 2: Push to external Remote %REMOTE%)
    rem *** AUSKOMMENTIERT: HIER ENDET DER BARE-REPO-TEIL ***
    rem #######################################################

    rem echo.
    rem WARNING: In the next step, a force-push to REMOTE %REMOTE% will be executed.
    rem if /I "y"=="y" (
    rem     echo Pushing Branch %BRANCH% to %REMOTE% --force ...
    rem     git push %REMOTE% %BRANCH% --force
    rem     if errorlevel 1 (
    rem         echo Push of Branch to %REMOTE% failed.
    rem         timeout /t 222
    rem         exit /b 1
    rem     )

    rem     rem Push Tags
    rem     echo Pushing Tag %TAG_NAME% to %REMOTE%...
    rem     git push %REMOTE% "%TAG_NAME%"
    rem     if errorlevel 1 (
    rem         echo Push of Tag to %REMOTE% failed.
    rem         timeout /t 222
    rem         exit /b 1
    rem     )

    rem     echo  ..
    rem     echo ########################################
    rem     echo Force-push and Tag Push %prj% to github REMOTE %REMOTE% successful.
    rem     echo  ..


    rem ) else (
    rem     echo REMOTE Push aborted. No push executed.
    rem )

) else (
    echo No changes to commit.
)

:ENDE
timeout /t 222
ENDLOCAL
exit /b 0
