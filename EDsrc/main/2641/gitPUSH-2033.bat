...
set "version=gitpushtim_%num:~0,3%.%num:~3,3%.%num:~6,3%"
...
if errorlevel 1 (
    ...
    rem *** AUSKOMMENTIERT: HIER BEGINNT DER BARE-REPO-TEIL ***
    rem #######################################################

    rem echo WARNING: Now pushing to local Bare Repo %BARE_DIR%.
    rem if /I "y"=="y" (
    rem     echo Pushing Branch %BRANCH% to %BARE_DIR% ...
    rem     git push "%BARE_DIR%" %BRANCH%
    rem     if errorlevel 1 (
    rem         echo Push of Branch to BARE_DIR failed.
    rem         timeout /t 222
    rem         exit /b 1
    rem     )

    rem     rem Push Tags
    rem     echo Pushing Tag %TAG_NAME% to %BARE_DIR%...
    rem     git push "%BARE_DIR%" "%TAG_NAME%"
    rem     if errorlevel 1 (
    rem         echo Push of Tag to BARE_DIR failed.
    rem         timeout /t 222
    rem         exit /b 1
    rem     )
    ...
    rem     echo  ..
    rem     echo ########################################
    rem     echo Push and Tag Push to BARE_DIR %prj% successful.
    rem     echo  ..

    rem ) else (
    rem     echo BARE Push aborted.
    rem )
