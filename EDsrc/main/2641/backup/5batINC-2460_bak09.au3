...
Func AppendFiles(ByRef $arFil)
  ...
  ; AppendFiles(ByRef $arFil)
    ...
    ; 1. Zielpfad extrahieren und erstellen, falls nicht vorhanden
    Local $sTargetDir = StringTrimRight(StringReplace($sTarget, FileGetName($sTarget), ""), 1)
    If Not DirExists($sTargetDir) Then
        DirCreate($sTargetDir)
        If @error Then Return SetError(1, 0, False) ; Fehler beim Erstellen des Verzeichnisses
    EndIf

    ; 2. Zieldatei loeschen, falls sie existiert (um sauberen Start zu gewaehrleisten)
    If FileExists($sTarget) Then
        FileDelete($sTarget)
        If @error Then Return SetError(2, 0, False) ; Fehler beim Loeschen der Datei
    EndIf

    ; 3. Datei im APPEND-Modus oeffnen (Modus 1, da sie gerade geloescht wurde, funktioniert dies wie ein WRITE)
    Local $hFile = FileOpen($sTarget, 1) ; Modus 1 = Append (Standardmodus, der bevorzugt wird)

    If $hFile = -1 Then Return SetError(3, 0, False) ; Fehler beim Oeffnen

    ; 4. Sequentielle Verarbeitung der Inhalte
    ...
    For $i = 2 To UBound($arFil) - 1 Step 2
        ...
        If $sIndicator = 'n' Then
            ...
            If @error Then ContinueLoop ; Überspringe, falls Datei nicht lesbar
...
Func AppendFiles(ByRef $arFil)
    ...
    For $i = 2 To UBound($arFil) - 1 Step 2
        ...
        If $sIndicator = 'n' Then
            ...
            ; Verwende FileWriteMy im APPEND-Modus (1), um den Inhalt anzuhaengen
            FileWriteMy($sTarget, $sFileData, 1)
        ElseIf $sIndicator = 's' Then
            ; Indikator 's': $sContent ist ein direkter String-Inhalt -> Schreibe ihn
            ; Verwende FileWriteMy im APPEND-Modus (1), um den String anzuhaengen
            FileWriteMy($sTarget, $sContent, 1)
        EndIf
    Next
