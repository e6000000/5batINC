...
;
;
#cs 	;--fuc  save2file from script  --fuc addINCbat   merge: copy script + incBAT1 + 2 + 3 +incBat4  script
...
#ce
#cs
; der praeprocessor: sucht auch im comment hahahahahaha:
!->Includefile "bat1-2040.inc" not found.
!->Includefile "bat2-2040.inc" not found.
!->Includefile "bat3-2040.inc" not found.
!->Includefile "bat4-2040.inc" not found.
!->Includefile "bat5-2040.inc" not found.
#ce
...
we have 6x .bat
and   6x .inc.au3 with the array building au3 code , every cmd.bat file line is to array
nicht cmd.bat bearbeiten sondern im au3 prj comit msg edit
; Target file (versioned)
Global $sBat1_Cpy = $sWorkingDirSLASH & "\bareFETCH-" & $verOUT & ".bat"
Global $sBat2_Cpy = $sWorkingDirSLASH & "\barePUSH-" & $verOUT & ".bat"
Global $sBat3_Cpy = $sWorkingDirSLASH & "\gitFETCH-" & $verOUT & ".bat"
Global $sBat4_Cpy = $sWorkingDirSLASH & "\gitPUSH-" & $verOUT & ".bat"
Global $sBat5_Cpy = $sWorkingDirSLASH & "\gitwebCREATE-" & $verOUT & ".bat"


ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') :  = ' &  & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console



#ce 	;--fuc  save2file from script  --fuc addINCbat   merge: copy script + incBAT1 + 2 + 3 +incBat4  script
...
Func Main()
	...
	Global $sInc1_Path   ;;// = $sWorkingDirSLASH & "\bat1-" & $iver & ".inc"
	Global $sInc2_Path ;;// = $sWorkingDirSLASH & "\bat2-" & $iver & ".inc"
	Global $sInc3_Path ;;// = $sWorkingDirSLASH & "\bat3-" & $iver & ".inc"
	Global $sInc4_Path ;;// = $sWorkingDirSLASH & "\bat4-" & $iver & ".inc"
	Global $sInc5_Path ;;// = $sWorkingDirSLASH & "\bat5-" & $iver & ".inc"
